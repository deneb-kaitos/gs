...
export class Handlers {
  ...
  constructor(redisConfig, streamOpts, sinkStreamName, serverId, debuglog) {
    ...
    this.#debuglog(`${this.constructor.name}::redisConfig`, this.#redisConfig);
    this.#debuglog(`${this.constructor.name}::streamOpts`, this.#streamOpts);
    this.#debuglog(`${this.constructor.name}::sinkStreamName`, this.#sinkStreamName);
    this.#debuglog(`${this.constructor.name}::serverId`, this.#serverId);
  }
  ...
  async message(ws, msg, isBinary) {
    ...
    this.#debuglog('.message: preparing streamMessage', msg);
    ...
    try {
      await this.#redisClient.executeIsolated(async (isolatedClient) => {
        this.#debuglog(
          `.message::isolatedClient.xAdd:
            ${this.#sinkStreamName},
            ${this.#AUTOGENERATED_ID},
            ${JSON.stringify(streamMessage)},
            ${JSON.stringify(this.#streamOpts)}`,
        );

        return await isolatedClient.xAdd(this.#sinkStreamName, this.#AUTOGENERATED_ID, streamMessage, this.#streamOpts);
      });
    } catch (err) {
      this.#debuglog('[ER] .message::isolatedClient', err);
    }

    // this.#debuglog(`[BEFORE] .message::.xAdd: ${this.#sinkStreamName}, ${this.#AUTOGENERATED_ID}, ${streamMessage}, ${this.#streamOpts}`);
    // await this.#redisClient.xAdd(this.#sinkStreamName, this.#AUTOGENERATED_ID, streamMessage, this.#streamOpts);
    // this.#debuglog(`[AFTER] .message::.xAdd: ${this.#sinkStreamName}, ${this.#AUTOGENERATED_ID}, ${streamMessage}, ${this.#streamOpts}`);

    this.#debuglog({
      id: (this.#clientsProperties.get(ws)).id,
      streamMessage,
      isBinary,
    });
